...
async def run_write_and_answer(
    fake_bot, session_factory, settings, llm, user, usage, alerter
) -> FSMContext:
...
async def test_answer_renders_correction_and_creates_error_cards(
    ...
    state = await run_write_and_answer(
        fake_bot, session_factory, settings, llm, user, usage, alerter
    )
...
async def test_error_cards_deduped_across_runs(
    fake_bot, session_factory, settings, user, usage, alerter
):
    from tests.fakes import FakeLLM

    llm = FakeLLM(correct_results=[correction(), correction()])
    await run_write_and_answer(fake_bot, session_factory, settings, llm, user, usage, alerter)
    await run_write_and_answer(fake_bot, session_factory, settings, llm, user, usage, alerter)
...
async def test_daily_cap_limits_error_cards(
    fake_bot, session_factory, settings, user, usage, alerter
):
    from tests.fakes import FakeLLM
...
async def test_llm_failure_keeps_state_for_retry(
    fake_bot, session_factory, settings, user, usage, alerter
):
    from tests.fakes import FakeLLM
...
async def test_no_errors_congratulates(fake_bot, session_factory, settings, user, usage, alerter):
    from tests.fakes import FakeLLM

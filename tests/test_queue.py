...
async def test_due_cards_come_first_ordered_by_due(session_factory):
    ...
    async with session_factory() as session:
        queue = await build_queue(session, now=now(), tz=TZ, session_max=30, daily_new_limit=15)
...
async def test_session_max_caps_due_cards(session_factory):
    ...
    async with session_factory() as session:
        queue = await build_queue(session, now=now(), tz=TZ, session_max=3, daily_new_limit=15)
...
async def test_new_cards_fill_remaining_slots_up_to_daily_limit(session_factory):
    ...
    async with session_factory() as session:
        queue = await build_queue(session, now=now(), tz=TZ, session_max=30, daily_new_limit=4)
    assert queue.due_count == 2
    assert queue.new_count == 4  # daily_new_limit wins over remaining slots

    async with session_factory() as session:
        queue = await build_queue(session, now=now(), tz=TZ, session_max=5, daily_new_limit=15)
    assert queue.due_count == 2
...
async def test_daily_new_limit_counts_cards_introduced_today(session_factory):
    ...
    for i in range(3):
        ...
        async with session_factory() as session:
            ...
            await repo.apply_review(
                session, card, result, rating=3, now=fixed_now - timedelta(hours=2)
            )
    ...
    async with session_factory() as session:
        queue = await build_queue(session, now=fixed_now, tz=TZ, session_max=30, daily_new_limit=4)
...
async def test_yesterdays_introductions_do_not_count(session_factory):
    ...
    async with session_factory() as session:
        card = await repo.get_card(session, card_id)
        result = ReviewResult(fsrs=card.fsrs, due=card.due, state=card.state, elapsed_days=0.0)
        await repo.apply_review(session, card, result, rating=3, now=now() - timedelta(days=2))
        await session.commit()
    for i in range(5):
        await add_vocab_card(session_factory, f"nouveau-{i}")

    async with session_factory() as session:
        queue = await build_queue(session, now=now(), tz=TZ, session_max=30, daily_new_limit=3)
